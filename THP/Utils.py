...
def event_loss(lambda_t,c):
    ...
    for i in range(len(lambda_t)):
        if i != c:
            los_loss += torch.log(1-(lambda_t[i]/torch.sum(lambda_t)))
    loss = los_loss + event_loss

    return loss

# def seqence_loss(e,e_p,e_N):
...
if __name__ == '__main__':
    d = 10
    K = 5
    e = torch.rand([2,10]) # [batch_size,d]
    e_p = torch.rand([2,10]) # [batch_size,d]
    e_N = torch.rand([2,5,10]) # [batch_size,k,d]
